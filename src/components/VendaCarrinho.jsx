...
const VendaCarrinho = ({ 
  ...
  const [descontosIndividuais, setDescontosIndividuais] = useState(0);
  ...
  useEffect(() => {
    const updateTotal = async () => {
      ...
      // Calcular soma dos descontos individuais
      const somaDescontosIndividuais = carrinho.reduce((acc, item) => 
        acc + (parseFloat(item.discount) || 0), 0
      );
      setDescontosIndividuais(somaDescontosIndividuais);
    };
    updateTotal();
    ...
    <div className="mt-4">
      ...
      <div className="mt-4 space-y-2">
        ...
        <div className="space-y-1">
          <p className="text-xl">Descontos Individuais: R$ {descontosIndividuais.toFixed(2)}</p>
          <p className="text-xl">Desconto Geral: R$ {desconto.toFixed(2)}</p>
          <p className="text-xl">Total de Descontos: R$ {(descontosIndividuais + parseFloat(desconto || 0)).toFixed(2)}</p>
